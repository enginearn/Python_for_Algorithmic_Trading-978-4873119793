...
class SMAVectorBacktester(object):
    ...
    SMA2: int
        time window in days fro longer SMA
    start: str
        start date for date retrieval
    end: str
        end date for date retrieval
    ...
    def __repr__(self):
        return "SMAVectorBacktester(symbol=%s, SMA1=%s, SMA2=%s, start=%s, end=%s)" % (
            self.symbol, self.SMA1, self.SMA2, self.start, self.end)
    ...
    def update_and_run(self, SMA):
        """ Updates SMA parameters and returns the negative absolute performance
        of the strategy (for minimization algorithm).

        Parameters
        ==========
        SMA: tuple
            SMA parameters (SMA1, SMA2)
        """
        self.set_parameters(SMA1=int(SMA[0]), SMA2=int(SMA[1]))
...
if __name__ == "__main__":
    smabt = SMAVectorBacktester("EUR=", 42, 252, "2010-01-01", "2020-12-31")
